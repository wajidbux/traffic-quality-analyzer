...
export function classifyRiskSync(probability) {
    if (probability === null || Number.isNaN(probability))
        return "lower";
    const b = cachedBands ?? { elevatedFrom: 0.5, highFrom: 0.75, veryHighFrom: 0.9 };
    if (probability >= b.veryHighFrom)
        return "very_high";
    if (probability >= b.highFrom)
        return "high";
    if (probability >= b.elevatedFrom)
        return "elevated";
    return "lower";
}
export async function classifyRisk(probability, bands) {
    if (probability === null || Number.isNaN(probability))
        return "lower";
...
export async function riskBandsWithRanges() {
...
export const RISK_BAND_ORDER = ["lower", "elevated", "high", "very_high"];
export async function describeBand(probability) {
...
export function histogramBin(probability) {
