...
import { buildReportPayload, buildSspPayload, exportAllCsv, generatePdf, generateResultsCsv, generateSspPdf, generateXlsx, recordReportAudit, } from "../services/reportGenerator.js";
import { getRun } from "../services/analyzer.js";
import { asyncHandler } from "../utils/asyncHandler.js";
export function reportsRouter(_context) {
    const router = Router();
    async function requireRun(runId) {
        if (!(await getRun(runId)))
            return { ok: false, status: 404, error: "Analysis run not found." };
        return { ok: true };
    }
    router.get("/:runId/pdf", asyncHandler(async (req, res) => {
    ...
    router.get("/:runId/xlsx", asyncHandler(async (req, res) => {
        const check = await requireRun(req.params.runId);
        if (!check.ok)
            return res.status(check.status).json({ error: check.error });
        try {
    ...
    router.get("/:runId/csv", asyncHandler(async (req, res) => {
        const check = await requireRun(req.params.runId);
        if (!check.ok)
            return res.status(check.status).json({ error: check.error });
    ...
    router.get("/:runId/ssp", asyncHandler(async (req, res) => {
        const check = await requireRun(req.params.runId);
        if (!check.ok)
            return res.status(check.status).json({ error: check.error });
