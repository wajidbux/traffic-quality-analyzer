...
export function settingsRouter(context) {
    const router = Router();
    ...
    router.post("/retention/purge", asyncHandler(async (req, res) => {
        ...
        const runIds = (await getDb()
            .prepare(`SELECT DISTINCT r.id FROM analysis_runs r
           WHERE r.started_at < ? AND NOT EXISTS (SELECT 1 FROM traffic_records t WHERE t.timestamp >= ?)`)
            .all(cutoff, cutoff));
        const oldRecords = (await getDb().prepare("SELECT id FROM traffic_records WHERE timestamp < ?").all(cutoff));
