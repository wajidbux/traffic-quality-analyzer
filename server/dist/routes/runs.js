...
import { ANALYSIS_MODES, QuotaGuardError, cancelRun, getRun, listRuns, previewAnalysis, startAnalysis, } from "../services/analyzer.js";
import { sampleStrategySchema } from "../services/samplingService.js";
import { getDb } from "../db/database.js";
import { audit } from "../services/auditLogger.js";
import { asyncHandler } from "../utils/asyncHandler.js";
export function runsRouter(context) {
    const router = Router();
    ...
    router.post("/preview", asyncHandler(async (req, res) => {
        ...
        if (!parsedMode)
            return res.status(400).json({ error: `Invalid analysis mode. Use one of: ${ANALYSIS_MODES.join(", ")}` });
    ...
    router.post("/", asyncHandler(async (req, res) => {
        ...
        if (confirmed !== true) {
            return res.status(409).json({
                error: "Analysis not started: confirmation is required. Preview the batch first, then send confirmed: true to run it.",
            });
        }
        try {
...
function parseMode(mode) {
    return typeof mode === "string" && ANALYSIS_MODES.includes(mode) ? mode : null;
}
