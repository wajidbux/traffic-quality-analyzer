 ...
 * absent.
 */
export function extractFraudProbability(body) {
    if (!isRecord(body))
        return null;
...
export function extractQuotaState(body, httpStatus, latencyMs) {
    let record = {};
    if (isRecord(body))
        record = body;
    ...
    const quotaObj = isRecord(record["quota"])
        ? record["quota"]
        : isRecord(record["data"]) && isRecord(record["data"]["quota"])
            ? record["data"]["quota"]
            : {};
...
export function looksLikeQuotaExhaustion(body) {
    if (!isRecord(body))
        return false;
