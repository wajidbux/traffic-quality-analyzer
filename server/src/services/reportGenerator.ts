...
import { getRun, RunSummary, riskBandCounts, RiskBand } from "./analyzer.js";
import {
  runRiskDistribution,
  channelStats,
  temporalAnalysis,
...
import { riskBandLabel, classifyRisk, classifyRiskSync } from "./riskClassifier.js";
import { audit } from "./auditLogger.js";

export const METHODOLOGY_STATEMENT =
  "This report analyzes a sampled set of CTV/Roku traffic signals using Pixalate's Ad Fraud API. " +
...
async function maskingEnabled(): Promise<boolean> {
...
export interface ReportPayload {
  runId: string;
  run: RunSummary;
  distribution: Array<{ bin: string; count: number; pct: number }>;
  bands: Array<{ band: RiskBand; count: number; pct: number }>;
  channels: Array<{ channelId: number; channelName: string; isActive: boolean; totalRecords: number; uniqueIps: number; uniqueRidas: number; averageRisk: number | null; medianRisk: number | null; highRiskPct: number | null; veryHighRiskPct: number | null; lowerRiskPct: number | null; elevatedRiskPct: number | null; pixalateFailures: number; countryDistribution: Array<{ country: string; count: number }>; deviceAgentDistribution: Array<{ label: string; count: number }>; resultsAnalyzed: number }>;
  temporal: Array<{ bucket: string; requests: number; avgRisk: number | null; highRiskPct: number | null }>;
  geo: Array<{ country: string; requests: number; avgRisk: number | null; highRiskPct: number | null; veryHighRiskPct: number | null }>;
  highRisk: Array<{ recordId: string; requestId: string | null; timestamp: string | null; channel: string | null; ip: string | null; rida: string | null; userAgent: string | null; country: string | null; probability: number | null; band: RiskBand; signalsSubmitted: string[]; responseStatus: number | null }>;
  comparison: Array<{ requestId: string; recordId: string; channel: string | null; timestamp: string | null; ipScore: number | null; deviceScore: number | null; uaScore: number | null; ipDeviceScore: number | null; ipDeviceUaScore: number | null; bestScore: number | null; signalsAvailable: number }>;
  ...
  methodology: string;
  masking: boolean;
  riskBands: { elevatedFrom: number; highFrom: number; veryHighFrom: number };
}

export async function buildReportPayload(runId: string): Promise<ReportPayload> {
...
async function maskedValue(value: string | null, kind: "ip" | "device" | "ua"): Promise<string> {
  if (!value) return "—";
  if (!(await maskingEnabled())) return value;
  if (kind === "ip") return maskIp(value) ?? "—";
  if (kind === "device") return maskDeviceId(value) ?? "—";
...
export async function generatePdf(payload: ReportPayload): Promise<Buffer> {
  ...
  const drawHeader = () => {
    ...
    doc.fontSize(9).fillColor("#6B7280").text(
      `Analysis Run ID: ${payload.runId}  •  Run started: ${payload.run.startedAt}  •  Report generated: ${payload.generatedAt}`
    );
    doc.moveDown(0.5);
  };
  ...
  table(
    doc,
    ["Band", "Count", "%"],
    payload.bands.map((b) => [riskBandLabel(b.band), String(b.count), b.pct.toFixed(1)])
  );
  doc.moveDown(0.4);
  ...
  if (payload.channels.length) {
    ...
    table(
      doc,
      ["Channel", "Records", "Unique IPs", "Unique RIDAs", "Avg risk", "High-risk %", "Failures"],
  ...
  if (payload.highRisk.length === 0) {
    ...
    const rows = await Promise.all(
      payload.highRisk.slice(0, 30).map(async (h) => [
        h.channel ?? "—",
  ...
  doc.fontSize(9).fillColor("#374151").text(
    `This analysis used ${run.apiCallsUsed} Pixalate Ad Fraud API call(s). Quota before: ${run.quotaBefore ?? "unknown"}, after: ${run.quotaAfter ?? "unknown"}. ` +
  ...
  doc.fontSize(8).fillColor("#9CA3AF").text(
    "Risk bands used in this report are analytical categories for triage purposes. " +
      `Elevated ≥ ${payload.riskBands.elevatedFrom.toFixed(2)}, High ≥ ${payload.riskBands.highFrom.toFixed(2)}, Very high ≥ ${payload.riskBands.veryHighFrom.toFixed(2)}.`,
    { indent: 10 }
  );
...
function table(doc: PDFKit.PDFDocument, headers: string[], rows: Array<string[]>, opts: { fontSize?: number } = {}): void {
  ...
  for (const row of rows) {
    let maxHeight = 12;
    const cells: Array<{ text: string; height: number }> = row.map((cell, i) => {
...
function drawTableHeader(doc: PDFKit.PDFDocument, headers: string[], colWidth: number, pad: number, size: number): void {
  const margin = 48;
  doc.fontSize(size).fillColor("#6B7280");
  headers.forEach((h, i) => {
    doc.text(h, margin + i * colWidth, doc.y, { width: colWidth - pad, height: 14 });
...
async function resultsDetailRows(runId: string): Promise<Array<Record<string, unknown>>> {
  const db = getDb();
  const rows = (await db
    .prepare(
      `SELECT pr.fraud_probability, pr.status, pr.signals_submitted, pr.analysis_mode,
  ...
  return rows.map((r) => ({
      ...
      signals: (JSON.parse(r.signals_submitted ?? "[]") as string[]).join("+"),
      status: r.status,
    }))
}

function riskBandLabelForValue(probability: number): string {
  return riskBandLabel(classifyRiskSync(probability));
}

// ---- CSV -------------------------------------------------------------------

export async function generateResultsCsv(runId: string): Promise<string> {
  const db = getDb();
  const rows = (await db
    .prepare(
      `SELECT r.ad_request_id, r.timestamp, c.name AS channel, r.ip, r.rida, r.device_id, r.user_agent,
  ...
  for (const row of rows) {
    lines.push(
      header
        .map((h) => {
          let value = row[h];
          if (h === "ip" && masking) value = maskIp(String(value));
          if ((h === "rida" || h === "device_id") && masking) value = maskDeviceId(String(value));
          if (h === "user_agent" && masking) value = maskUserAgent(String(value), 120);
          return esc(value);
        })
        .join(",")
    );
...
export interface SspReportPayload {
  runId: string;
  channel: string | null;
  sampleSize: number;
  period: string;
...
export async function buildSspPayload(runId: string, channelName?: string | null): Promise<SspReportPayload> {
  const run = await getRun(runId);
  if (!run) throw new Error(`Analysis run ${runId} not found`);
...
export async function generateSspPdf(payload: SspReportPayload): Promise<Buffer> {
  ...
  table(
    doc,
    ["Risk category", "% of sampled traffic"],
    [
  ...
  LIMITATIONS.forEach((l) => doc.text(`• ${l}`, { indent: 10 }));
  doc.moveDown(0.3);
  doc.fontSize(8.5).fillColor("#374151").text(
    "Risk categories are analytical bands defined for this analysis and are not determinations of fraud for any individual request."
  );
...
export async function recordReportAudit(kind: "full" | "ssp", runId: string): Promise<void> {
    ...
    lines.push(
      header
        .map((h) => {
          let value = row[h];
          if (h === "ip" && masking) value = maskIp(String(value));
          if (h === "rida" && masking) value = maskDeviceId(String(value));
          if (h === "user_agent" && masking) value = maskUserAgent(String(value), 120);
          return esc(value);
        })
        .join(",")
