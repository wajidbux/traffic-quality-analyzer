...
import { PixalateApiError, PixalateClientLike, PixalateFraudRequest } from "../pixalate/types.js";
import { validateSignals } from "../utils/validation.js";
import { TokenBucket, Semaphore } from "../utils/rateLimiter.js";
import { classifyRisk, RiskBand, RiskBandCount } from "./riskClassifier.js";
import { sampleRecords, SampleFilter, SampleStrategy } from "./samplingService.js";
import {
...
const MODE_SIGNALS: Record<Exclude<AnalysisMode, "auto">, Array<"ip" | "device" | "ua">> = {
  ip: ["ip"],
  device: ["device"],
  ua: ["ua"],
  ip_device: ["ip", "device"],
...
const AUTO_PRIORITY: Array<Array<"ip" | "device" | "ua">> = [
  ["ip", "device", "ua"],
  ["ip", "device"],
  ["ip", "ua"],
  ["device", "ua"],
...
function resolveSignalsForMode(
  mode: AnalysisMode,
  valid: { ip: string | null; deviceId: string | null; useragent: string | null }
): Array<"ip" | "device" | "ua"> {
  const available: Array<"ip" | "device" | "ua"> = [];
  if (valid.ip) available.push("ip");
  if (valid.deviceId) available.push("device");
  if (valid.useragent) available.push("ua");

  if (mode === "auto") {
    for (const combo of AUTO_PRIORITY) {
      if (combo.every((s) => available.includes(s))) return combo;
    }
    return [];
  }
...
export async function getRun(runId: string): Promise<RunSummary | null> {
  const row = (await getDb().prepare("SELECT * FROM analysis_runs WHERE id = ?").get(runId)) as
...
function mapRun(row: Record<string, unknown>): RunSummary {
  const toNum = (v: unknown): number | null => (typeof v === "number" && Number.isFinite(v) ? v : v === null ? null : Number(v));
  return {
    id: String(row.id),
...
export async function startAnalysis(
  ...
  if (!request.ignoreQuota && preview.insufficientQuota) {
    throw new QuotaGuardError(
      `Analysis blocked: estimated ${preview.estimatedCalls} Pixalate calls but only ${currentQuota} remaining. ` +
  ...
  await getDb()
    .prepare(
      `INSERT INTO analysis_runs (id, name, status, channel_filter, mode, sample_strategy, started_at,
  ...
  try {
    ...
    const insertError = db.prepare(
      `INSERT INTO api_errors (occurred_at, endpoint, http_status, error_code, error_message, record_id, run_id)
       VALUES (datetime('now'), 'analysis', ?, ?, ?, ?, ?)`
    );
    const updateProgress = db.prepare(
      "UPDATE analysis_runs SET progress_processed = ?, num_success = ?, num_failed = ?, api_calls_used = ? WHERE id = ?"
    );
    ...
    const processOne = async (recordId: string): Promise<void> => {
      ...
      await semaphore.run(async () => {
        ...
        if (controller.signal.aborted) return;
        processed += 1;
        const record = (await getRecord.get(recordId)) as Record<string, unknown> | undefined;
        if (!record) return;

        const { valid, dropped } = validateSignals({
          ip: record.ip as string | null,
          rida: record.rida as string | null,
          device_id: record.device_id as string | null,
        ...
        if (signals.length === 0) {
          ...
          await insertResult.run(
            ...
            422,
            "no_signals",
            `No requested signals available for mode ${request.mode}${dropped.length ? ` (dropped: ${dropped.map((d) => d.signal).join(", ")})` : ""}`,
            null
          );
          return;
        }

        const payload: PixalateFraudRequest = {};
        if (signals.includes("ip") && valid.ip) payload.ip = valid.ip;
        if (signals.includes("device") && valid.deviceId) payload.deviceId = valid.deviceId;
        if (signals.includes("ua") && valid.useragent) payload.useragent = valid.useragent;
        ...
        try {
          ...
          waveFailed += 1;
          waveCalls += 1;
          const error = err instanceof PixalateApiError ? err : new PixalateApiError("network", String((err as Error)?.message ?? err));
    ...
    const finalRow = (await getDb().prepare("SELECT * FROM analysis_runs WHERE id = ?").get(runId)) as Record<string, unknown>;
    ...
    await getDb()
      .prepare(
        `UPDATE analysis_runs SET status = ?, ended_at = ?, progress_processed = ?, num_processed = ?,
      ...
      .run(
        ...
        quotaAfter,
        aggregates.avgRisk,
        aggregates.medianRisk,
        aggregates.pctLower,
        aggregates.pctElevated,
        aggregates.pctHigh,
        aggregates.pctVeryHigh,
        stoppedEarly
    ...
    logger.info(
      { runId, status, success, failed, callsUsed },
...
export async function computeAggregates(runId: string): Promise<{
  avgRisk: number | null;
  medianRisk: number | null;
  pctLower: number | null;
  pctElevated: number | null;
  ...
  const rows = (await db
    ...
    .all(runId)) as Array<{ fraud_probability: number | null }>;
  const values = rows
  ...
  for (const v of values) {
    ...
    if (band === "lower") lower += 1;
    else if (band === "elevated") elevated += 1;
    else if (band === "high") high += 1;
    else veryHigh += 1;
  }
  const pct = (n: number) => (n / values.length) * 100;
  return {
...
export type { RiskBand, RiskBandCount } from "./riskClassifier.js";

export async function riskBandCounts(runId: string): Promise<RiskBandCount[]> {
  const db = getDb();
  const rows = (await db
    .prepare(
      "SELECT fraud_probability FROM pixalate_results WHERE run_id = ? AND status = 'success' AND fraud_probability IS NOT NULL"
  ...
  const counts: Record<RiskBand, number> = { lower: 0, elevated: 0, high: 0, very_high: 0 };
  for (const r of rows) counts[await classifyRisk(r.fraud_probability)] += 1;
  const total = rows.length || 1;
  return (["lower", "elevated", "high", "very_high"] as RiskBand[]).map((band) => ({
    band,
    count: counts[band],
