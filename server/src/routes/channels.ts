...
export function channelsRouter(_context: AppContext): Router {
  const router = Router();
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const channel = (await getDb().prepare("SELECT * FROM channels WHERE id = ?").get(id)) as
        | Record<string, unknown>
        | undefined;
      if (!channel) return res.status(404).json({ error: "Channel not found." });
      const nextName = typeof name === "string" && name.trim() ? name.trim() : (channel as { name: string }).name;
      const nextDescription = description === undefined ? (channel as { description: string | null }).description : description;
      const nextActive = is_active === undefined ? (channel as { is_active: number }).is_active : (is_active ? 1 : 0);
      await getDb()
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      await audit("channel.deactivate", "channels", String(id), { name: (channel as { name: string }).name });
