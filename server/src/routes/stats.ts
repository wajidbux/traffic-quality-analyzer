...
export function statsRouter(_context: AppContext): Router {
  const router = Router();
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (runId) return res.json({ distribution: await runRiskDistribution(runId) });
      const kpis = await dashboardKpis();
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      const runId = typeof req.query.runId === "string" ? req.query.runId : null;
      if (!runId) return res.status(400).json({ error: "runId is required for temporal analysis." });
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      const runId = typeof req.query.runId === "string" ? req.query.runId : null;
      if (!runId) return res.status(400).json({ error: "runId is required for geo analysis." });
