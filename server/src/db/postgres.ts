...
/** Anything with a `query(sql, params)` method — a Pool or a checked-out client. */
interface Queryable {
  query(sql: string, params?: unknown[]): Promise<{ rows: Row[]; rowCount: number | null }>;
}

function statementsFor(queryable: Queryable, sql: string): Statement {
  ...
  return {
    ...
    run: async (...params: unknown[]): Promise<RunResult> => {
      ...
      return {
        changes: rowCount ?? 0,
        lastInsertRowid:
          row !== undefined && typeof row.id !== "undefined" ? (row.id as number | bigint | null) : null,
      };
    },
  };
}
...
export class PostgresDatabase implements Database {
  ...
  async transaction<T>(fn: (tx: Database) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      const tx: Database = {
        prepare: (s: string) => statementsFor(client, s),
        transaction: () => {
